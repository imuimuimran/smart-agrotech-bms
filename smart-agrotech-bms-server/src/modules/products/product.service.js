...
import { ACTIVITY_ACTIONS, ACTIVITY_MODULES } from "../activity-logs/activityLog.constants.js";
import {
...
const createProduct = async (payload, reqUser) => {
  ...
  // Authoritative Logging Point: Trigger ONLY after database write succeeds
  await ActivityLogService.logActivity({
    user: reqUser._id || reqUser.id, // Internal MongoDB ObjectId reference tracking
    action: ACTIVITY_ACTIONS.CREATE,
    module: ACTIVITY_MODULES.PRODUCT,
    entityId: product._id, // References the target entity ID trace
...
const updateProduct = async (productId, updateData, reqUser) => {
  ...
  // Authoritative Logging Point: Trigger ONLY after database modifications update successfully
  await ActivityLogService.logActivity({
    user: reqUser._id || reqUser.id,
    action: ACTIVITY_ACTIONS.UPDATE,
    module: ACTIVITY_MODULES.PRODUCT,
    entityId: product._id,
    description: `Updated product ${product.publicId}`.trim(),
    metadata: {
      publicId: product.publicId,
